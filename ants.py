    ...
    def nearest_bee(self, hive):
        ...
        while i <=self.max_range:
            ...
            if end==hive:
                break
            i=i+1
        def nearest_neighbor(place,hive):
            if place==hive:
                return None
            if place.bees:
                return place.bees
            else:
                return nearest_neighbor(place.entrance,hive)
...
#        else:
        #print('DEBUG: ',start, end,self.place,hive)
        return random_or_none(nearest_neighbor(start,end))
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
            ...
            #print('DEBUG: ', 'IS imposter=',self.is_imposter)
            ...
            while place:
                if place.ant:
                    if place.ant not in self.buffed:
                        #print('DEBUG: ',place,place.ant,self.buffed)
                        place.ant.damage*=2
                        self.buffed.append(place.ant)
                    if place.ant.is_container and place.ant.contained_ant:
                        if place.ant.contained_ant not in self.buffed:
                            place.ant.contained_ant.damage*=2
                            self.buffed.append(place.ant.contained_ant)
...
#    start_time=-1
#    dur=bee.slow_turns
#    print('DEBUG: ', 'init',bee.slow_turns)
'''    def new_action(colony):

        start_time=colony.time
        if bee.slow_turns<=0:

#            print('DEBUG: ', bee.slow_turns)
            action(colony)
            print('DEBUG: ', '***using_old_action_Position',bee.place.name)
            return
        print('DEBUG: ','slow action','slow_turns=',bee.slow_turns)
    #    print('DEBUG: ','slow action','scare_turns=',bee.scare_turns)
        if colony.time%2==0:
            print('DEBUG: ', 'even time',action)
            old_turn=bee.slow_turns
            action(colony)
            bee.slow_turns=old_turn
            print('DEBUG: ', '***Position',bee.place.name)



        bee.slow_turns-=1
        print('DEBUG: ', 'end_action_slow_turns=',bee.slow_turns)

    #    print('DEBUG: ', 'end_action_scare_turns=',bee.scare_turns)
...
#            print('DEBUG: ', bee.slow_turns)
    #        action(colony)
    #        return
        #print('DEBUG: ','scare action','slow_turns=',bee.slow_turns)
        #print('DEBUG: ','scare action','scare_turns=',bee.scare_turns)
...
#        print('DEBUG: ', bee.scare_turns,bee.scared)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    if effect==make_scare and bee.been_scared==True:
        return
    if effect==make_scare and bee.been_scared==False:
        bee.been_scared=True
    original_bee_action = bee.action
    affected_bee_action = effect(original_bee_action,bee)

    def changed_action_duration(colony):
    	nonlocal duration
    	duration -= 1
    	if duration >= 0:
    		affected_bee_action(colony)
    	else:
    		original_bee_action(colony)

    bee.action = changed_action_duration
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        distance=0
        place=self.place
        while place!=None and place!=hive:
            if place.ant:
                if  place.ant!=self:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return max(0,self.damage-0.05*self.insects_shot-0.2*distance)
        # END Problem OPTIONAL
